...
plot(rollmean$Date,log(2)/rollmean$mean_7,type='o',pch=16,las=1,ylab='Doubling time (7-day windows)',xlab='Date',cex.lab=1.2,ylim=c(1.8,7))
...
states <- ggplot(data=covid_by_state,aes(x=DaysSince25, y=(log(Cases)) - (log(starting_value)),color=Province.State))  + 
  ...
  coord_cartesian(xlim =c(0.5,35),ylim =c(-0.1,8)) +
...
covid_by_state <- covid_by_state %>%
  group_by(Province.State) %>%
  filter(length(Cases)>3) %>%
  mutate(first25_confirmed = format(Date[which(Cases>24)[1]],format = "%j"),  first100_confirmed = format(Date[which(Cases>99)[1]],format = "%j")) %>%
  group_by(Province.State) %>%
  mutate(starting_value = min(Cases)) %>%
  mutate(DaysSince25 = as.Date(Date,format='%m.%d.%y')- as.Date(first25_confirmed,format='%m.%d.%y'), DaysSince100 = as.Date(Date,format='%m.%d.%y')-as.Date(first100_confirmed,format='%m.%d.%y')) %>%
  mutate(first25_confirmed = as.numeric(first25_confirmed), first100_confirmed = as.numeric(first100_confirmed)) 
...
all_data_by_state$RestrictBusinesses <- as.numeric(all_data_by_state$RestrictBusinesses<all_data_by_state$first100_confirmed) 
all_data_by_state$RestrictBusinesses[is.na(all_data_by_state$RestrictBusinesses)] = 0
all_data_by_state$StayAtHome <- as.numeric(all_data_by_state$StayAtHome<all_data_by_state$first100_confirmed) 
...
# Build plot
par(mfrow=c(2,3),mar=c(4,0.3,1,0),oma=c(0,5,0.5,0.5))
...
boxplot(all_data_by_state$doubling_time~all_data_by_state$RestrictRestaurants,las=1,xlab='Restrict restaurants by day of 25 cases',xaxt='n',yaxt='n')
...
boxplot(all_data_by_state$doubling_time~all_data_by_state$RestrictBusinesses,las=1,xlab='Restrict businesses by day of 100 cases',xaxt='n')
#text(2,5.2,labels = '*significant difference')
axis(side = 1,at = 1:2,labels = c('No','Yes'))
points(all_data_by_state$RestrictBusinesses+1,all_data_by_state$doubling_time,pch=16)
boxplot(all_data_by_state$doubling_time~all_data_by_state$StayAtHome,las=1,xlab='Stay at home order by day of 100 cases',xaxt='n',yaxt='n')
axis(side = 1,at = 1:2,labels = c('No','Yes'))
points(all_data_by_state$StayAtHome+1,all_data_by_state$doubling_time,pch=16)

boxplot(all_data_by_state$doubling_time~all_data_by_state$total_score,las=1,xlab='Number of state government actions',yaxt='n')
#axis(side = 1,at = 1:2,labels = c('No','Yes'))
...
```{r, finding_best_model,echo=F,results='asis',warning=FALSE,message=FALSE,error=FALSE}
require(MASS)
